...
const GlobalCSS = () => (
  <style>{`
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:${c.border};border-radius:10px}
...
/* ═══════════════════════ HEADER BAR (greeting + weather combined) ═══════════════════════ */
            ...
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                ...
                <div style={{ fontSize: 10, color: c.dim }}>
                  H:{Math.round(w.high || 0)}° L:{Math.round(w.low || 0)}° · {Math.round(w.wind_speed_10m)} mph · North Reading
                </div>
              </div>
            </>
          )}
...
function WeekView({ gcal, weekOffset }) {
  ...
  const [newEv, setNewEv] = useState({ title: "", startTime: "", endTime: "", allDay: false });
  ...
  const addTodo = (dk) => {
    const text = (inputs[dk] || "").trim();
    if (!text) return;
    setTodos(p => ({ ...p, [dk]: [...(p[dk] || []), { id: uid(), text, done: false }] }));
    setInputs(p => ({ ...p, [dk]: "" }));
  };
  ...
  const addEvent = (dk) => {
    ...
    setLocalEvents(ev => [...ev, {
      id: uid(), title: newEv.title.trim(),
      start: newEv.allDay ? `${dk}T00:00:00` : `${dk}T${newEv.startTime || "09:00"}:00`,
      end: newEv.endTime && !newEv.allDay ? `${dk}T${newEv.endTime}:00` : null,
      allDay: newEv.allDay, source: "local",
    }]);
    setNewEv({ title: "", startTime: "", endTime: "", allDay: false });
    setAddingEvent(null);
  };
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 6, height: "100%" }}>
      {weekDates.map(date => {
        ...
        return (
          <div key={dk} style={{
            background: isToday ? c.brownSoft : c.surfaceAlt,
            border: `1px solid ${isToday ? c.brownMed : c.border}`,
            borderRadius: 10, padding: 10, display: "flex", flexDirection: "column",
            minHeight: 0, overflow: "hidden", transition: "border-color 0.2s",
                ...
                {dayEv.map(ev => (
                  <div key={ev.id} style={{ display: "flex", gap: 5, alignItems: "flex-start", padding: "2px 0", fontSize: 11 }}>
                    ...
                    <div style={{ flex: 1, lineHeight: 1.3 }}>
                      ...
                      {!ev.allDay && <div style={{ fontSize: 9, color: c.dim }}>{fmt(ev.start)}</div>}
                    </div>
            ...
            <div style={{ flexShrink: 0, marginBottom: 4 }}>
              {addingEvent === dk ? (
                <div style={{ animation: "slideDown 0.15s ease" }}>
                  <input placeholder="Event title" value={newEv.title}
                    onChange={e => setNewEv(n => ({...n, title: e.target.value}))}
                    onKeyDown={e => { if (e.key === "Enter") addEvent(dk); if (e.key === "Escape") setAddingEvent(null); }}
                    autoFocus
                    style={{ width: "100%", background: "transparent", border: `1px solid ${c.borderLight}`, borderRadius: 5, padding: "3px 6px", color: c.text, fontSize: 10, outline: "none", marginBottom: 3, fontFamily: "inherit" }} />
                  <div style={{ display: "flex", gap: 3 }}>
                    <Btn variant="primary" onClick={() => addEvent(dk)} style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4 }}>Add</Btn>
                    <Btn variant="ghost" onClick={() => setAddingEvent(null)} style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4 }}>Cancel</Btn>
                  </div>
                </div>
              ) : (
                <div onClick={() => setAddingEvent(dk)} style={{
                  fontSize: 9, color: c.dim, cursor: "pointer", padding: "2px 0", transition: "color 0.15s",
                  display: "flex", alignItems: "center", gap: 3,
                }} onMouseEnter={e => e.currentTarget.style.color = c.blue} onMouseLeave={e => e.currentTarget.style.color = c.dim}>
                  <Svg d={I.plus} size={9} /> event
                </div>
              )}
            </div>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, overflowY: "auto", minHeight: 0 }}>
              {active.map(todo => (
                <div key={todo.id} style={{ display: "flex", alignItems: "flex-start", gap: 5, fontSize: 11, lineHeight: 1.35, padding: "2px 0" }}>
                  <div onClick={() => toggleTodo(dk, todo.id)} style={{
                    width: 14, height: 14, borderRadius: 3, border: `1.5px solid ${c.borderLight}`, flexShrink: 0, cursor: "pointer", marginTop: 1, transition: "border-color 0.15s",
              ...
              {done.map(todo => (
                <div key={todo.id} style={{ display: "flex", alignItems: "flex-start", gap: 5, fontSize: 11, lineHeight: 1.35, padding: "2px 0", opacity: 0.35 }}>
                  <div onClick={() => toggleTodo(dk, todo.id)} style={{
                    width: 14, height: 14, borderRadius: 3, background: c.greenSoft, border: `1.5px solid ${c.green}40`,
                    flexShrink: 0, cursor: "pointer", marginTop: 1, display: "flex", alignItems: "center", justifyContent: "center",
                  }}><Svg d={I.check} size={8} color={c.green} /></div>
                  <span style={{ flex: 1, textDecoration: "line-through", color: c.dim, wordBreak: "break-word" }}>{todo.text}</span>
                </div>
              ))}
            </div>

            <input placeholder="+ task" value={inputs[dk] || ""}
              onChange={e => setInputs(p => ({ ...p, [dk]: e.target.value }))}
              onKeyDown={e => e.key === "Enter" && addTodo(dk)}
              style={{
                marginTop: 4, background: "transparent", border: `1px solid ${c.border}`, borderRadius: 6,
                padding: "4px 7px", color: c.text, fontSize: 10, outline: "none", width: "100%", transition: "border-color 0.15s", flexShrink: 0, fontFamily: "inherit",
              }}
...
function AIChat() {
  ...
  const sendMessage = async () => {
    ...
    try {
      let assistantText = "";
      const sysPrompt = "You are a helpful, concise assistant embedded in a productivity dashboard. Keep responses brief and actionable. Use short paragraphs.";

      if (provider === "anthropic") {
        const res = await fetch("https://api.anthropic.com/v1/messages", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-api-key": apiKey, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" },
          body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1024, system: sysPrompt, messages: newMessages.map(m => ({ role: m.role, content: m.content })) }),
        });
        const data = await res.json();
        ...
        const res = await fetch("https://api.openai.com/v1/chat/completions", {
          ...
          body: JSON.stringify({ model: "gpt-4o-mini", max_tokens: 1024, messages: [{ role: "system", content: sysPrompt }, ...newMessages.map(m => ({ role: m.role, content: m.content }))] }),
        });
        const data = await res.json();
        assistantText = data.choices?.[0]?.message?.content || data.error?.message || "Something went wrong.";
        ...
        const data = await res.json();
        assistantText = data.candidates?.[0]?.content?.parts?.[0]?.text || data.error?.message || "Something went wrong.";
      }
  ...
  const clearChat = () => { setMessages([]); store.save("ai-chat", []); };
    ...
    <div style={{
      ...
      <div style={{ padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${c.border}`, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: c.sub }}>
            {apiKey ? providerLabel() : "AI Assistant"}
          </span>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          ...
          {messages.length > 0 && <Btn variant="ghost" onClick={clearChat}><Svg d={I.trash} size={11} /></Btn>}
        </div>
      </div>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8, minHeight: 0 }}>
        {messages.length === 0 && !loading && (
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 6, color: c.dim }}>
            <Svg d={I.bot} size={22} color={c.border} />
            <span style={{ fontSize: 12 }}>{apiKey ? "Ask me anything" : "Add an API key to start"}</span>
          </div>
        )}
        {messages.map((m, i) => (
          <div key={i} style={{
            ...
            background: m.role === "user" ? c.brownMed : c.surfaceAlt,
            border: `1px solid ${m.role === "user" ? c.brown + "30" : c.border}`,
            fontSize: 13, lineHeight: 1.55, color: c.text, whiteSpace: "pre-wrap", wordBreak: "break-word",
          }}>{m.content}</div>
        ))}
        {loading && (
          <div style={{ alignSelf: "flex-start", padding: "8px 11px", borderRadius: "11px 11px 11px 3px", background: c.surfaceAlt, border: `1px solid ${c.border}` }}>
            <span style={{ fontSize: 12, color: c.dim, animation: "blink 1s infinite" }}>thinking...</span>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: "8px 12px", borderTop: `1px solid ${c.border}`, display: "flex", gap: 6, flexShrink: 0 }}>
        <input value={input} onChange={e => setInput(e.target.value)}
          ...
          placeholder={apiKey ? "Type a message..." : "Set API key first..."} disabled={!apiKey}
          style={{
            flex: 1, background: c.input, border: `1px solid ${c.border}`, borderRadius: 8,
            padding: "8px 11px", color: c.text, fontSize: 13, outline: "none", transition: "border-color 0.15s",
            opacity: apiKey ? 1 : 0.5, fontFamily: "inherit",
          }}
          onFocus={e => e.target.style.borderColor = c.blue}
          onBlur={e => e.target.style.borderColor = c.border} />
        <Btn onClick={sendMessage} style={{ padding: "8px 12px", opacity: (!input.trim() || !apiKey) ? 0.4 : 1 }}>
          <Svg d={I.send} size={14} color="#fff" />
        </Btn>
      </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, animation: "fadeIn 0.15s ease" }} onClick={() => setShowKeyModal(false)}>
          <div style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: 14, padding: 24, width: 400, maxWidth: "90vw", animation: "slideDown 0.25s ease" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h3 style={{ fontSize: 16, fontWeight: 700 }}>AI Setup</h3>
              <Btn variant="icon" onClick={() => setShowKeyModal(false)}><Svg d={I.x} size={16} /></Btn>
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 12, color: c.sub, marginBottom: 6, display: "block" }}>Provider</label>
              <div style={{ display: "flex", gap: 5 }}>
                {[["anthropic", "Claude"], ["openai", "GPT"], ["gemini", "Gemini"]].map(([k, label]) => (
                  <button key={k} onClick={() => setProvider(k)} style={{
                    flex: 1, padding: "8px 10px", borderRadius: 8, border: `1px solid ${provider === k ? c.brown : c.border}`,
                    background: provider === k ? c.brownSoft : "transparent", color: provider === k ? c.brownLight : c.sub,
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="password"
                ...
                style={{ width: "100%", background: c.input, border: `1px solid ${c.border}`, borderRadius: 8, padding: "10px 13px", color: c.text, fontSize: 13, outline: "none", fontFamily: "inherit" }} />
              <p style={{ fontSize: 11, color: c.dim, marginTop: 6, lineHeight: 1.5 }}>
                {provider === "gemini" ? "Get your key from aistudio.google.com. Stored locally only." : `Stored locally. Sent only to ${provider === "anthropic" ? "Anthropic" : "OpenAI"}'s API.`}
              </p>
            </div>
...
function QuickNotes() {
  ...
  const add = () => {
    ...
    setNotes(n => [{ id: uid(), text: input.trim(), createdAt: Date.now() }, ...n]);
    setInput("");
  };
  ...
  return (
    <div style={{
      background: c.surface, border: `1px solid ${c.border}`, borderRadius: 12,
      padding: 12, display: "flex", flexDirection: "column", height: "100%", overflow: "hidden",
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 8, flexShrink: 0 }}>
        <input value={input} onChange={e => setInput(e.target.value)}
          ...
          style={{
            flex: 1, background: c.input, border: `1px solid ${c.border}`, borderRadius: 7,
            padding: "6px 10px", color: c.text, fontSize: 12, outline: "none", fontFamily: "inherit", transition: "border-color 0.15s",
          }}
          onFocus={e => e.target.style.borderColor = c.blue}
          onBlur={e => e.target.style.borderColor = c.border} />
      ...
      <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4, minHeight: 0 }}>
        {notes.map(n => (
          <div key={n.id} style={{
            ...
            background: c.surfaceAlt, borderLeft: `2px solid ${c.brownLight}40`,
            display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 6,
          }}>
            <span style={{ flex: 1, wordBreak: "break-word", color: c.white }}>{n.text}</span>
            <span onClick={() => remove(n.id)} style={{ cursor: "pointer", color: c.dim, fontSize: 9, opacity: 0.4, flexShrink: 0 }}>✕</span>
          </div>
        ))}
...
export default function App() {
  ...
  return (
    <div style={{ height: "100vh", background: c.bg, padding: "14px 18px", display: "flex", flexDirection: "column", overflow: "hidden", gap: 10 }}>
      <GlobalCSS />
      ...
      <div style={{ display: "flex", gap: 10, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <WeekView gcal={gcal} weekOffset={weekOffset} />
        </div>
